...
class UserTest < ActiveSupport::TestCase
  ...
  test "the name should not be too long" do 
    @user.name = random_letter * 51
    assert_not @user.valid?
  end

  test "the email should not be too long" do
    @user.email = random_letter * 244 + "@example.com"
    assert_not @user.valid?
  end
  ...
  test "password should have a minimum length" do
    @user.password = @user.password_confirmation = random_letter * 5
    assert_not @user.valid?
  end

  # Auxiliary method to generate random letters.
  def random_letter 
    ('a'..'z').to_a.shuffle[1].to_s
  end
end
